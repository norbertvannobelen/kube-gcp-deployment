...
# Start the components:
  ${GSSH}${instance} -- sudo mv crio.service kubelet.service kube-proxy.service /etc/systemd/system/
  ${GSSH}${instance} -- sudo systemctl daemon-reload
  ${GSSH}${instance} -- sudo systemctl enable crio kubelet kube-proxy
  ${GSSH}${instance} -- sudo systemctl start crio kubelet kube-proxy
...
function addKubeComponents() {
  ...
  kubectl create -f https://raw.githubusercontent.com/kubernetes/kubernetes/master/cluster/saltbase/salt/l7-gcp/glbc.manifest
